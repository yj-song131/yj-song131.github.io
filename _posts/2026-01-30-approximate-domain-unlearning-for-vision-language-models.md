...
- 유지해야 할 클래스는 cross-entropy loss를 최소화
$$L_{forget}(B) = -\frac{1}{|B|} \sum_{i=1}^{|B|} \sum_{j=1}^{|C|} \frac{1}{|C|} \log p_{ij}$$

- 잊어버려야 할 클래스는 분포를 uniform하게 만듦(=entropy 최대화) 
...
- 이 MMD loss에 -를 붙여 최대화 → Intra-domain divergence를 maximize
- domain들끼리 멀리 떨어지도록
...
- 이 과정을 거치면 원래의 고정된 프롬프트가 현재 이미지의 특징을 반영하여 Instance-wise Prompt로 업데이트
...
- attn map에서 잊혀져야 할 domain의 주의를 성공적으로 분산시켰다 ? 
