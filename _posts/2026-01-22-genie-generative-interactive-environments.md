...
excerpt: "ICML 2024"
---
>ICML 2024. [[Paper]](https://arxiv.org/abs/2402.15391) [[Github]](https://deepmind.google/models/genie/)   
...
- 연속된 두 프레임 $t$, $t+1$을 관찰
- 다음 프레임 $x_{t+1}$을 예측할 때 이전 프레임 $t$에서 어떤 action이 취해졌는지를 condition으로 줌
- $x_{1:t}$(지금까지의 모든 프레임)과 바로 다음 프레임 $x_{t+1}$을 비교해서 encoder를 통과 → 사이의 변화를 설명하는 latent action 집합 $\tilde{a}_{1:t}$가 나옴
- $x_{1:t}$와 latent action $a$를 decoder에 입력하면 예측된 다음 프레임 값인 $\hat{x}_{t+1}$이 출력
- 예측 x와 실제 x 간의 Reconstruction loss를 Minimize

<img src="/assets/images/notion-image-1770724141748-n1dv1l.png" style="width: 50%; display: block; margin: 0 auto;">
...
<img src="/assets/images/notion-image-1770724147992-ts0qtm.png" style="width: 50%; display: block; margin: 0 auto;">
...
<img src="/assets/images/notion-image-1770724148917-9ober.png" alt="Dynamics Model" style="width: 50%; display: block; margin: 0 auto;">
...
- 학습 때 input token $z$를 베르누이 분포에 따라 0.5~1 사이에서 균일하게 샘플링해 무작위로 마스킹
...
1. frame $x_1$을 모델에 입력 → tokenize되어 $z_1$

2. 범위 내의 정수 값을 선택해 수행할 latent action $a_1$을 지정 → VQ codebook에서 인덱싱하여 $\tilde{a}_1$ 획득

3. Dynamics model이 $z_1$, $\tilde{a}_1$을 사용하여 다음 프레임 토큰 $\hat{z}_2$ 예측

4. 이걸 autoregressive하게 반복 …

모델을 두가지 방식으로 사용할 수 있음

1. Re-generation: 실제 영상을 시작 프레임+action 써서 다시 만들어보기

2. New Trajectories: 새로운 action을 넣으면 완전히 새로운 플레이 영상을 만들어냄
...
<img src="/assets/images/notion-image-1770724153858-1yi233.png" style="width: 70%; display: block; margin: 0 auto;">

imagen2로 생성한 결과→ 배경은 거의 움직임 X

<img src="/assets/images/notion-image-1770724155368-fmv6jr.png" alt="imagen2 result" style="width: 50%; display: block; margin: 0 auto;">

서로 다른 프레임에서 시작했을 때 trajectory 예시

<img src="/assets/images/notion-image-1770724156682-ql933l.png" alt="imagen2 result" style="width: 50%; display: block; margin: 0 auto;">
